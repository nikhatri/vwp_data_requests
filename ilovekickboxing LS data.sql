...
--join SALESFORCE_MINDBODY_PREP.ACCOUNT
--on (try_cast(meeting_events.STUDIOID as integer) = account.site_id__c and logo_account__C is not null)

WHERE
      --account.name = 'iLoveKickboxing' AND
...
meeting_events.STUDIOID, mbaccountnumber, date_trunc('week', meeting_events."CREATEDATE"), meeting_events."CREATEDATE"
